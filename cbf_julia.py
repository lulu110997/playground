    ...
    for row in xb_locs:
        ...
        if REPLAY:
            ...
            for idx in range(1, STEPS):
                ...
                rs = np.array([*x_opt_curr, *obstacles[0]]), np.array([*x_opt_curr, *obstacles[1]])
                ...
                for o in range(len(obstacles)):
                    alpha, da_dp = get_α_J(rs[o], qs)
                    # https://github.com/BolunDai0216/DifferentiableOptimizationCBF/issues/6#issuecomment-2708300188
                    G_opt[o] = -np.array(da_dp[:7])  # CBF derivative, assuming that object 1 is the ee
                    # print(G_opt); sys.exit()
                    h_opt[o] = GAMMA * (alpha - BETA) # CBF exponential gamma*hx
